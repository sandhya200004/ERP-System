...
const ReflectiveCard: React.FC<ReflectiveCardProps> = ({
  employeeName = 'ALEXANDER DOE',
  employeeTitle = 'SENIOR DEVELOPER',
  employeeId = '8901-2345-6789',
  blurStrength = 12,
  color = 'white',
  metalness = 1,
  roughness = 0.4,
  ...
  return (
    ...
    >
      ...
      {!enableWebcam && (
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-br from-gray-700 via-gray-800 to-gray-900 z-0" />
      )}
      ...
      <div className="relative z-10 h-full flex flex-col justify-between p-8 text-[var(--text-color,white)] bg-[var(--overlay-color,rgba(255,255,255,0.05))]">
        ...
        <div className="flex-1 flex flex-col justify-end items-center text-center gap-6 mb-8">
          <div className="text-center">
            <h2 className="text-2xl font-bold tracking-[0.05em] m-0 mb-2 drop-shadow-md">{employeeName.toUpperCase()}</h2>
            <p className="text-xs tracking-[0.2em] opacity-70 m-0 uppercase">{employeeTitle}</p>
          </div>
        </div>
        ...
        <div className="flex justify-between items-end border-t border-white/20 pt-6">
          <div className="flex flex-col gap-1">
            ...
            <span className="font-mono text-sm tracking-[0.05em]">{employeeId}</span>
          </div>
